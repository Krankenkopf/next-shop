import React, { ChangeEvent, CSSProperties, DetailedHTMLProps, InputHTMLAttributes, PropsWithChildren, ReactElement, useCallback, useEffect, useState } from "react"
...
export const Toggle = <TValue extends string>({
    type, name, titles, options, value, className, frameClassName, onChange, onChangeOption, children, ...restProps
    ...
    useEffect(() => {
        ...
        if (currentIndex === valueIndex && path.length) {
            setCounter(1)
            timerId = setTimeout(() => { setPath([]) }, TRANSITION_TIME/path.length)
        }
        return () => clearTimeout(timerId)
    }, [counter])

    const onChangeCallback = (e: ChangeEvent<HTMLInputElement>) => {
        if (!path.length) { // prevent change when switching is not completed
            onChange && onChange(e)
    ...
    const getTransitionStyle = useCallback(() => {
        let divider = path.length ? path.length : 1
        return ({
            ...
            return (
                <li key={option}>
                    ...
                    <label htmlFor={option}>
                        ...
                        <div className={getFrameStyle()} style={getTransitionStyle()}></div>
                    </label>
                </li>
            )
        })
