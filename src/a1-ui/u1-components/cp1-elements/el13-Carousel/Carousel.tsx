...
export const Carousel: FC<TCarouselProps> = ({ items, itemsPerView, arrows, arrowStep = itemsPerView, transitionTime = itemsPerView * 100 }) => {
    ...
    const [firstSlideItems, midSlideItems, secondToLastSlideItems, lastSlideItems] = useMemo(() => {
        let first = [], mid = [], secondtolast = [], last = []
        for (let i = 0; i < items.length; i++) {
            if (i < itemsPerView) {
                // FIRST VIEW SET
                first.push(<li key={i + Math.random()} style={{ flex: `1 0 ${100 / itemsPerView}%`, }}
                    className={css.carousel__item__container}>
                    <div className={css.carousel__item}>
                        {items[i]}
                    </div>
                </li>)
                if (i >= itemsPerView * (fullSlidesAmount - 1) && fullWidth <= 2) {
                    // SECOND-TO-LAST VIEW SET
                    secondtolast.push(<li key={i} style={{ flex: `1 0 ${100 / itemsPerView}%`, }}
                        className={css.carousel__item__container}>
                        <div className={css.carousel__item}>
                            {items[i]}
                        </div>
            ...
            } else if (i >= (isNoPartialSlide ? itemsPerView * (fullSlidesAmount - 1) : itemsPerView * fullSlidesAmount)) {
                // LAST VIEW SET
                last.push(<li key={i} style={{ flex: `1 0 ${100 / itemsPerView}%`, }}
                    className={css.carousel__item__container}>
                    <div className={css.carousel__item}>
                        {items[i]}
                    </div>
                ...
                // REST VIEW SETS
                mid.push(<li key={i} style={{ flex: `1 0 ${100 / itemsPerView}%`, }}
                    className={css.carousel__item__container}>
                    <div className={css.carousel__item}>
                        {items[i]}
                    </div>
    ...
    const controlDots = useMemo(() => {
        ...
        for (let i = 0; i < (isNoPartialSlide ? fullWidth : Math.ceil(fullWidth)); i++) {
            ...
            if (i === (isNoPartialSlide ? fullWidth-1 : Math.floor(fullWidth)) && i !== 0) {
                ...
                dots.push(<li key={Math.floor(fullWidth)} value={fullWidth - 1}>
                    <input
    ...
    return <div className={css.carousel}>
        <div style={{ padding: (arrows && items.length > itemsPerView) ? "0 1.5em" : "0" }}
            ...
            {arrows && items.length > itemsPerView && <div className={css.carousel__controls__arrows}>
                <div data-value={"-1"} onClick={onArrowClick}>{arrows[0]}</div>
                <div data-value={"1"} onClick={onArrowClick}>{arrows[1]}</div>
            </div>}
        </div>
