...
import { Icon, Toggle, Button } from '../../../elements';
...
export const ProductDetail: FC<TProductDetail> = ({
  ...
  useEffect(() => {
    const arr: Array<Ail> = [];
    currentArticle &&
      currentArticle.galleryDetails.forEach(item => {
  ...
  return (
    <article className="product-detail__wrapper">
      {imgs.length && (
        <figure className="product-detail__mainImg">
      ...
      <aside>
        <div className="product-detail__sidebar">
          ...
          {isInCart && (
            <p className="product-detail__sidebar__cart-info text center">In your cart</p>
          )}
      ...
      {imgs.length > 1 && (
        <section className="product-detail__otherImgs">
          {imgs.map((img, i) => {
            if (i !== 0)
              return (
                <figure key={img.url}>
